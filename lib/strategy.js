...
JwtStrategy.prototype.authenticate = function(req, options) {
  ...
  var done = function(secretOrKey_err, secretOrKey) {
    if (secretOrKey_err) return self.fail(secretOrKey_err)
    // Verify the JWT
    JwtStrategy.JwtVerifier(token, secretOrKey, this._verifOpts, function(jwt_err, payload) {
      if (jwt_err) {
  ...
  if (typeof this._secretOrKey === 'function') {
    this._secretOrKey(token, done, req)
  } else {
    done(null, this._secretOrKey)
  }
};
